...
export default function MerchantRequestsPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["asset-requests", "merchant", activeSpaceId],
    queryFn: () => listAssetRequests({ spaceId: activeSpaceId ?? undefined, requestType: "MERCHANT_ASSET", page: 1, limit: 50 }),
  ...
  return (
    <div className="space-y-6">
      <PageHeader eyebrow="Requests" title="Merchant requests" description="Requests that flow through zonal manager review and warehouse fulfillment." />
      <Card>
        <CardContent className="space-y-3 p-6">
          {!activeSpaceId ? <div className="rounded-2xl border border-orange-100 bg-orange-50/40 p-4 text-sm text-slate-600">Select a space to load merchant requests.</div> : isLoading ? <Skeleton className="h-32 w-full" /> : isError ? <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700">Merchant requests could not be loaded.</div> : requests.length ? requests.map((request) => (
          ...
          )) : <div className="rounded-2xl border border-orange-100 bg-orange-50/40 p-4 text-sm text-slate-600">No merchant requests found.</div>}
        </CardContent>
      </Card>
    </div>
  );
