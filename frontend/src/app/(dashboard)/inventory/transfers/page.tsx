...
function entityLabel(entity?: Entity | string | null) {
  ...
  return entity.name || entity.code || entity.sku || personName || entity.employeeId || entity.email || "";
}

function assetLabel(entry: TransferTransaction) {
...
export default function InventoryTransfersPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["inventory-transfers", activeSpaceId],
    queryFn: () => listInventoryTransactions({ spaceId: activeSpaceId ?? undefined, transactionType: "TRANSFER", page: 1, limit: 50 }),
    enabled: Boolean(activeSpaceId),
  });
  const transfers = getApiItems<TransferTransaction>(data);
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="space-y-3 p-6">
          {!activeSpaceId ? (
            <div className="rounded-2xl border border-orange-100 bg-orange-50/40 p-4 text-sm text-slate-600">Select a space to view transfers.</div>
          ) : isLoading ? (
            <Skeleton className="h-32 w-full" />
          ) : isError ? (
            <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700">Transfers could not be loaded.</div>
          ) : transfers.length ? (
            transfers.map((entry) => {
                  ...
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      ...
                      <p className="text-sm text-slate-600">
                        {entityLabel(entry.fromWarehouseId) || "Source"} → {entityLabel(entry.toWarehouseId) || "Destination"} · Qty {entry.quantity ?? 1}
                      </p>
                    </div>
            ...
            <div className="rounded-2xl border border-orange-100 bg-orange-50/40 p-4 text-sm text-slate-600">No transfer records found.</div>
          )}
        </CardContent>
      </Card>
    </div>
