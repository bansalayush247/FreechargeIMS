...
} = require("../repositories/userRole.repository");
...
const authorize = (...requiredPermissions) => {
  return async (req, res, next) => {
    try {
      ...
      // Handle system-level operations (e.g., CREATE_SPACE)
      if (isSystemLevelOperation && !spaceId) {
        ...
        // Admin users have all system-level permissions
        ...
        if (!userRoles.length) {
          logger.warn(
            `No roles found for user=${userId} space=${spaceId}`
          );
